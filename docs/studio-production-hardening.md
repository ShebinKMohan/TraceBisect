...
- Configured ClamAV scanning sends bounded upload bytes through clamd's
  `INSTREAM` protocol before parsing or persistence. Threats and scanner
  failures fail closed, readiness fails while a required scanner is
  unavailable, and the production Compose bundle keeps ClamAV private with a
  persistent signature volume.
- Uploaded bytes are written to a temporary file generated by the server, not to
  the user-supplied filename.
- API rate limiting is per client, method, and bounded action. Resource IDs and
  unknown paths collapse into fixed categories instead of creating bypassable
  buckets. Memory and SQLite modes use
  an exact process-local sliding window. PostgreSQL mode stores only SHA-256
  bucket identifiers and uses transaction advisory locks plus expiring shared
  buckets. PostgreSQL time is the shared clock, so every API replica applies one
  exact limit without storing raw client, path, or account keys.
