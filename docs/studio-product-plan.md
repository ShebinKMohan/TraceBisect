...
browser key lifecycle, centralized metrics/alerts, error tracking, and
distributed rate limiting remain separate SaaS milestones. The API already
provides operator-managed expiring keys and a protected Prometheus scrape
endpoint; hosting-platform collection is still required.
