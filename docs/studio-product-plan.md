...
browser key lifecycle, hosted monitoring/error-tracking wiring, and distributed
rate limiting remain separate SaaS milestones. The API already provides
operator-managed expiring keys, a protected Prometheus scrape endpoint, starter
alert rules, and an incident runbook; hosting-platform collection and delivery
are still required.
