...
**Primary signal — shared deterministic IDs.** When `baseline.source_event_id == candidate.source_event_id` (both traces derive from the same instrumented session, e.g. the same OTel exporter run at two code versions) or when both events carry the same `Event.id` (some recorders use deterministic step-keyed IDs like `"run42:step7"`), this is the strongest possible Tier 1 evidence. Equality is exact string comparison. It fires unconditionally with one exception for `TOOL_CALL` / `MCP_CALL`: when the two events call different tools (tool identity as defined in `spec/divergence-taxonomy.md` §3.5) and either tool still appears among the other side's siblings under the same wildcard rules (siblings with no known name part are ignored for this check), the pair is skipped and left to the identity-aware tiers below. Raw IDs can be position-derived — the native recorder emits `evt_NNN` — so trusting them across a reorder, insertion, or deletion would pair different steps. When neither tool appears on the other side, the raw-ID pair stands, so a genuine tool swap is still paired and reported by `changed_tool_args`.
...
19. **Position-derived raw IDs** — native-recorder IDs (`evt_002`, `evt_003`) on two sibling `TOOL_CALL`s with equal arguments, swapped in the candidate. Each raw-ID pair is skipped because both tools still appear on the other side; the Tier 1 `(tool_name, args_hash)` tuple pairs them. Expect two `ID_MATCH`es crossing `evt_002 ↔ evt_003` and no divergence. For distinctly named tools, a candidate that inserts or removes a tool before later siblings likewise yields one `INSERTION` / `DELETION` for that tool, not a chain of mismatched pairs. Same-name duplicates still pair by sibling order (§3), and Tier 2 keys on `semantic_name`, so changed arguments combined with a reorder can still pair the wrong calls. A single tool replaced by a tool absent from the baseline keeps its raw-ID `ID_MATCH`.
