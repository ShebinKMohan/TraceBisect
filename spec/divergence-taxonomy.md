...
`source_metadata` carries detector-level provenance — at minimum `{"detector": "<name>", "alignment_kind": "<MatchKind>"}`. It is the diagnostic escape hatch and is not interpreted by rendering or by `assert_aligned`.
